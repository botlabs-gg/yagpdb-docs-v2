...
This is an alternative way of `/rep give`.

### Name for reputation points

This will be displayed in the `/rep check` command.
By default, it is _Rep_.

### Rep cooldown in seconds

This is the cooldown for the `/rep give` and `/rep take` commands.
It does not affect the `/rep set` command.
No one can bypass this cooldown, including admins.

### &#x20;Maximum amount of rep that can be given/taken away in one command

This affects the `/rep give` and `/rep take` commands which users can choose to specify the number of points to be given/taken away.

### Admin role

Users with this role have access to the `/rep set` command and can freely change anyone's points to any number.

### Required role to give/remove points

Users with this role have access to the `/rep give` and `/rep take` commands which is affected by the _Rep cool down in seconds_ and the _Maximum amount of rep that can be given/taken away in one command_.
...
Users with this role cannot use the `/rep give` and `/rep take` commands.
...
Besides using the command `/rep log` , you can also check the logs from the control panel.
