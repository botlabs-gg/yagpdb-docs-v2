...
|`urlescape` "string"| Escapes the _string_ so it can be safely placed inside a URL path segment - e.g. "Hello, YAGPDB!" becomes "Hello%2C%20YAGPDB%21"<br>There's also predefined template package function `urlquery` which is covered in [Golang text/template documentation](https://pkg.go.dev/text/template#hdr-Functions).|
...
in the next section, further documentation can be found in [Golang formatting documentation](https://golang.org/pkg/fmt/).
